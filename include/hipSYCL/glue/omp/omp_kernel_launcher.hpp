...
template <class KernelNameTraits, int Dim, class Function>
...
inline void parallel_for_ndrange_kernel(
    ...
    size_t num_local_mem_bytes, hipsycl::ext::performance_api_guard& perf_api_guard) noexcept
{
  ...
  void bind(sycl::id<Dim> offset, sycl::range<Dim> global_range,
    ...
    this->_invoker = [=] (rt::dag_node* node) mutable {
      ...
      if constexpr(type == rt::kernel_type::single_task){
        ...
        if(!is_with_offset) {
          omp_dispatch::parallel_for_kernel<KernelNameTraits>(k, global_range, perf_api_guard);
        } else {
          omp_dispatch::parallel_for_kernel_offset<KernelNameTraits>(k, global_range, offset, perf_api_guard);
        }

      } else if constexpr (type == rt::kernel_type::ndrange_parallel_for) {

        omp_dispatch::parallel_for_ndrange_kernel<KernelNameTraits>(
            k, get_grid_range(), local_range, offset, dynamic_local_memory, perf_api_guard);
        ...
        omp_dispatch::parallel_for_workgroup<KernelNameTraits>(k, get_grid_range(), local_range,
                                             dynamic_local_memory, perf_api_guard);
